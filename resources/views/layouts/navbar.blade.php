...
<div id="mobile-drawer" class="fixed top-0 left-0 h-full w-[85%] max-w-[340px] bg-white z-50 overflow-y-auto lg:hidden" style="transform: translateX(-100%); transition: transform 0.3s ease-in-out;">
    ...
    <nav>
        @foreach ($headerMenu as $item)
                    ...
                    <div class="drawer-accordion-content hidden bg-gray-50">
                        ...
                        @else
                            @foreach ($item->children as $child)
                                @if ($child->route)
                                    <a href="{{ route($child->route) }}" class="block px-8 py-3 text-[14px] text-gray-700 border-b border-gray-100 hover:bg-gray-100">
                                        {{ $child->title }}
                                    </a>
                                @else
            ...
            @elseif ($item->route)
                <a href="{{ route($item->route) }}" class="flex items-center px-5 py-4 text-[15px] font-medium text-gray-900 border-b border-gray-200 hover:bg-gray-50">
                    {{ $item->title }}
                </a>
            @else
    ...
    <div class="px-5 pt-5 pb-2 border-b border-gray-200">
        ...
        @auth
        <div class="flex items-center gap-3 py-3 text-[14px] text-gray-800">
        ...
        <div class="flex items-center gap-3 py-3 text-[14px] text-gray-800">
            ...
            <span>
                <a href="javascript:void(0)" onclick="openLoginModal()" class="hover:underline">Login</a>
                <span class="mx-1">|</span>
...
<span>{{ $contactEmail }}</span>
            ...
            <div class="ml-auto flex items-center gap-5">
                ...
                <div class="hidden lg:flex items-center gap-2 text-sm">
                    ...
                    @auth
                    <a href="{{ route('account') }}" class="hover:text-[#C9F9B6]">My Account</a>
                    <span class="text-white/40">|</span>
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="hover:text-[#C9F9B6]">Logout</button>
                    </form>
                    @else
                    <a href="javascript:void(0)" onclick="openLoginModal()" class="hover:text-[#C9F9B6]">Login</a>
                    <span class="text-white/40">|</span>
                    <a href="{{ route('register') }}" class="hover:text-[#C9F9B6]">Register</a>
                    @endauth
                </div>
                        ...
                        @if ($hasChildren || $isIntro || $isBooks || $isCounselling)
                            <div class="relative desktop-dropdown-parent">
                                ...
                                <div class="desktop-dropdown">
                                    ...
                                    @else
                                        <div class="bg-[#232323] min-w-[200px] px-5 py-3 flex flex-col gap-3 items-start">
                                            @foreach ($item->children as $child)
                                                @if ($child->route)
                                                    <a href="{{ route($child->route) }}" class="relative inline-block text-white text-[14px] font-medium
                                                        after:absolute after:bottom-0 after:left-0 after:h-[1px] after:w-0
                                                        after:bg-[#C9F9B6] after:transition-all after:duration-300
                                                        hover:after:w-full">{{ $child->title }}</a>
                                                @else
                                                    <a href="{{ $child->url }}" class="relative inline-block text-white text-[14px] font-medium
                                                        after:absolute after:bottom-0 after:left-0 after:h-[1px] after:w-0
                                                        after:bg-[#C9F9B6] after:transition-all after:duration-300
                                                        hover:after:w-full">{{ $child->title }}</a>
                                                @endif
